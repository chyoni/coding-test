...
- 참고로, 자바의 HashMap도 이 방식을 사용한다. 예를 들어, "apple" 과 "banana"라는 다른 글자를 해시 함수를 돌렸는데 우연히 둘 다 같은 100이라는 
해시값이 나오면 "apple"을 키로 하는 값을 먼저 넣었다고 했을때 100번지에 "apple"에 대한 값이 저장된다. "banana"를 키로 하는 값을 이후에 또 넣으려고
하는데 또 100번이 나오면 100번지에가서 "apple"과 "banana"가 같은지 비교한다 (equals() 사용) 두 값이 다르다는 것을 판단하면 같은 번지에 연결 리스트로
매달아 저장한다. 헷갈리는 포인트는 이런 경우다. "어? 나는 A라는 키에 1과 2를 저장했더니 2만 나오는데요?" 이건 A라는 키 자체가 완전 동일하기 때문에 당연히
'충돌'이라고 바라보지 않고 '업데이트'라고 바라보는 것이다. 해시 충돌은 키가 다른데 같은 해시값이 나오는것을 말한다. 
...
> **로드 밸런싱이란 무엇이며, L4 로드 밸런서와 L7 로드 밸런서의 차이점에 대해 설명하라.**
```bash
[로드 밸런싱 정의]
하나의 서버에 집중되는 부하를 여러 대의 서버로 균등하게 분산하여 시스템의 가용성과 응답 속도를 높이는 기술

[L4와 L7 차이점]
핵심은 어떤 정보를 바탕으로 분산하느냐에 있다. 아래 표를 보자.
```
| 구분    | L4 로드 밸런서               | L7 로드 밸런서                              |
|-------|-------------------------|----------------------------------------|
| 계층    | 전송 계층 (Transport Layer) | 응용 계층 (Application Layer)              |
| 판단 근거 | IP 주소, 포트 번호 (TCP/UDP)  | URL, HTTP 헤더, 쿠키, 페이로드 등               |
| 특징    | 데이터 내용을 보지 않아 속도가 빠름    | 패킷 내용을 분석하므로 정교한 라우팅 가능                |
| 예시    | 단순히 80번 포트 요청을 서버들에 분산  | `/api/user`는 A서버로, `api/shop`은 B서버로 분산 |

**정리**

```bash
- L4: 데이터의 내용을 보지 않기 때문에 보안상 안전하고 IP/포트 기반으로 빠르고 단순해 효율적이지만, 특정 사용자의 요청을 특정 서버로 고정하기 어렵다.
- L7: HTTP 헤더나 쿠키를 읽을 수 있어 컨텐츠 기반 라우팅이 가능하다. 예를 들어, 특정 국가의 접속자를 가까운 서버로 보내거나,
이미지 파일 요청만 이미지 전용 서버로 보낼 수 있다.
```

**[만약, 서버가 여러 대라면 세션 관리는 어떻게 해야 할까?]**
```bash
- Sticky Session: 로드 밸런서가 특정 사용자의 요청을 항상 같은 서버로 보내도록 설정한다.
- Session Clustering: 서버들끼리 세션을 공유한다.
- Session Storage: Redis 같은 외부 저장소에 세션을 따로 보관한다. (가장 권장되는 방식)
```

---

> **우리가 브라우저 주소창에 `www.google.com` 을 입력하고 엔터를 치면, 우리 화면에 구글 페이지가 뜨기까지 네트워크 단에서 어떤 일들이 일어날까?**
```bash
1. 운영체제가 이 도메인의 실제 아이피를 알기 위해 먼저 캐시를 뒤지고, 없으면 DNS 서버에 물어본다.
2. IP 주소를 알아냈으니 이제 그 서버로 달려가서 TCP 3-Way Handshake를 수행한다.
  - SYN(클라이언트 -> 서버): "나 연결하고 싶어! 내 시작 번호는 100번이야 (Seq=100)"
  - SYN + ACK(서버 -> 클라이언트): "반가워! 연결 수락할게. 내 시작 번호는 500번(Seq=500)이고, 네 100번 잘 받았어. 다음엔 101번 보내줘(Ack=101)"
  - ACK(클라이언트 -> 서버): "확인했어! 네 500번 잘 받았어. 다음엔 501번 보내줘! (Ack=501)"
3. SSL/TLS Handshake
  - TCP 연결이 되자마자 바로 데이터를 주고받지 않는다. 보안을 위해 TLS Handshake를 한다.
  - 서버가 본인의 SSL 인증서를 보내면, 브라우저는 이게 진짜 구글인지 확인한다.
  - 이 과정에서 데이터를 암호화할 **대칭키**를 안전하게 교환한다.
4. HTTP Request & Response
  - 이제 터널이 열리고 브라우저가 요청을 보낸다.
  - Request: GET / index.html HTTP/1.1
  - Response: 서버가 HTML, CSS, JS 파일을 묶어서 보낸다.
5. Rendering
  - 브라우저가 받은 HTML을 읽어서 DOM 트리를 만들고, 우리가 보는 예쁜 구글 화면을 그려낸다.  
  
[몇가지 더 알고 있으면 좋은 것]
- Keep-Alive: 매번 3-Way Handshake를 하면 너무 느릴것이다. 그래서 한 번 뚫어놓은 TCP 연결을 재사용하는 기술
```

---

> **HTTP 메서드에서 "멱등성(Idempotency)"이란 무엇이며, 대표적인 메서드 (GET, POST, PUT, DELETE) 중 멱등성이 보장되는 것은 무엇인가?**
```bash
[멱등성 정의]
- 연산을 여러 번 수행해도 결과가 달라지지 않는 성질을 의미한다.
- 즉, 동일한 요청을 한 번 보내는 것과 여러 번 연속해서 보내는 것이 서버의 상태에 동일한 영향을 미친다면 그 메서드는 멱등하다.

[주요 메서드별 멱등성 여부]
- GET (멱등함 ✅): 서버의 상태를 변경하지 않고 조회만 하므로, 여러 번 호출해도 결과(서버 데이터)는 변하지 않는다.
- PUT (멱등함 ✅): 요청에 담긴 자원으로 기존 자원을 대체(Replace)한다. 같은 데이터를 여러 번 PUT해도 최종 결과는 항상 같다.
- DELETE (멱등함 ✅): 특정 자원을 삭제한다. 처음 삭제 시 자원이 사라지고, 그 뒤로 여러 번 호출해도 해당 자원이 삭제된 상태로 그대로 유지된다.
(응답 코드는 404로 바뀔 수 있지만 서버의 '상태'관점에서는 멱등하다)
- POST (멱등하지 않음 ❌): 호출할 때마다 새로운 자원을 생성할 수 있다. 예를 들어 결제 요청 POST를 3번 보내면 3번의 결제가 발생할 수 있으므로 
멱등하지 않는다.

참고로, PATCH는 구현방식에 따라 멱등할 수도 있고 아닐 수도 있다. 단순히 값을 특정 값으로 변경하는 경우에는 멱등하다.
그러나 age += 1과 같은 상대적인 연산을 수행하도록 설계했다면 호출할 때마다 값이 변하므로 멱등하지 않게 된다.

[실무에서 이게 중요한 이유]
- 네트워크 오류로 클라이언트가 응답을 못 받았을 때, 재시도(Retry)를 해도 안전한지 판단하는 기준이 된다. 결제와 같은
POST 요청은 멱등하지 않기 때문에 별도의 ID(Idempotency-Key)를 발급하여 중복 처리를 막는 설계가 필요하다. 
```